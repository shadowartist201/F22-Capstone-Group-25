    ...
    internal class BattleUI : Game
    {
        ...
        public static void DrawText(SpriteBatch _spriteBatch)
        {
            ...
            if (message_alpha == 1f) //message box
            {
                ...
                if (attack_message) //when attack message activated, draw it
                {
                    _spriteBatch.DrawString(Game1.large_font, "*" + Battle.characters[current_character] + " attacked the " + Game1.enemies[Battle.target - 1].name + "!", new Vector2(205, 361), Color.Black);
                }
                if (magic_message) //when magic message activated, draw it
                {
                    if (Game1.squad[current_character - 1].mana > 0)
                        _spriteBatch.DrawString(Game1.large_font, "*" + Battle.characters[current_character]+ " summoned fire!", new Vector2(205, 361), Color.Black);
                    else
                        _spriteBatch.DrawString(Game1.large_font, "*" + Battle.characters[current_character] + " is out of mana!", new Vector2(205, 361), Color.Black);
                }
                if (cat_magic_message) //when cat message message activated, draw it
                {
                    menu_alpha = 0f;
