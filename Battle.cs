    ...
    public class Battle : GameScreen
    {
        ...
        */
        ...
        public override void Draw(GameTime gameTime)
        {
            ...
            if (enemies[0].health == 0 || oldState.IsKeyDown(Keys.D1))
            {
                Entity thing = enemies[0];
                thing.health = 0;
                enemies[0] = thing;
            ...
            else  //if battle still going
            {
                ...
                if (message_alpha == 1f) //message box
                {
                    if (initial_message) //when initial message activated, draw it
                    {
                        ...
                        foreach(Entity e in enemies)
                        {
                            _spriteBatch.DrawString(Game1.medium_font, "*A(n) " + e.name + " appeared!", new Vector2(205,361+(21*i)),Color.Black);
                            i+=1;
                        }
                    ...
                    if (attack_message) //when attack message activated, draw it
                    {
                        ...
                        if (!alratk) //and not already attacked
                        {
                            ...
                            returned = attack(ref currchar, ref currenemy, _spriteBatch, battle_message, Game1.large_font, Game1.small_font);
                            squad[current_character - 1] = returned[0];
                            enemies[0] = returned[1];
                            alratk = true; //set attacked flag to true
                        }
                        _spriteBatch.Draw(battle_message, new Rectangle(182, 336, 299, 128), Color.White);
                        _spriteBatch.DrawString(Game1.large_font, "*" + characters[current_character-1] + " attacked the " + enemies[target-1] + "!", new Vector2(205,361), Color.Black);
                    }
                    if (magic_message) //when magic message activated, draw it
                    {
                        if (squad[current_character - 1].mana > 0) //if current character has MP
                        {
                            if (!alratk) //and not already attacked
                            {
                                ...
                                List<Entity> returned = new List<Entity>(); 
                                menu_alpha = 0f; //hide menu
                                returned = spattack(ref currchar, ref currenemy, _spriteBatch, battle_message, Game1.medium_font);
                                squad[current_character - 1] = returned[0];
                                enemies[0] = returned[1];
                                alratk = true; //set attacked flag to true
                            }
                        ...
                        {
                            _spriteBatch.Draw(battle_message, new Rectangle(182, 336, 299, 128), Color.White);
                            _spriteBatch.DrawString(Game1.large_font, "*" + characters[current_character] + " summoned fire!", new Vector2(205, 361), Color.Black);
                        }
                        else
                        {
                            _spriteBatch.Draw(battle_message, new Rectangle(182, 336, 299, 128), Color.White);
                            _spriteBatch.DrawString(Game1.large_font, "*" + characters[current_character] + " is out of mana!", new Vector2(205, 361), Color.Black);
                        }
                    }
                ...
                //dragon hp
                _spriteBatch.DrawString(Game1.medium_font, "Debug HP: " + enemies[0].health + " / " + enemies[0].mHealth, new Vector2(325, 217), Color.Cyan);
                ///TO DO:
