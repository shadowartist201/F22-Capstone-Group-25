...
namespace Game_Demo
{
    internal class BattleUIref : Game
    {
        ...
        public static void DrawText(SpriteBatch _spriteBatch)
        {
            ...
            if (message_alpha == 1f) //message box
            {
                ...
                if (attack_message) //when attack message activated, draw it
                {
                    _spriteBatch.DrawString(Game1.large_font, "*" + Game1.squad[current_character] + " attacked the " + Game1.enemies[Battle.target - 1].name + "!", new Vector2(205, 361), Color.Black);
                }
                if (magic_message) //when magic message activated, draw it
                {
                    if (Game1.squad[current_character - 1].mana > 0)
        ...
        {
            current_character += 1;
            if(current_character>Game1.squad.Count && squadTurn)
            {
                current_character = 0;
                squadTurn = false;
            }
            else if(current_character>Game1.enemies.Count && !squadTurn)
            {
                current_character = 0;
                squadTurn = true;
            }
